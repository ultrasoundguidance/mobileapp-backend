...
router.post('/sendWelcomeEmailToNewSubscriber', async (req, res) => {
  ...
  if (member.previous.status === 'free') {
    ...
    <p>Don't hesitate to send us an email with any questions, comments, or concerns. We're here to help! If you like what you see, give us a 1-3 sentence review on the app store or send the review our way. We always love to hear your thoughts!</p>
    <hr>
...
router.post('/sendWelcomeEmailToFreeMobileMember', async (req, res) => {
  ...
  if (member.current.labels.find((item) => item.name === 'Mobile app signup')) {
    ...
    <p>The free tier of Ultrasound Guidance gives you access to a few introductory videos so you can get a feel for what we're all about. We hope you like what you see! When you're ready to level up your ultrasound skills, check out the <a href="https://www.ultrasoundguidance.com/plans/">MSK Complete Package</a>!</p>
